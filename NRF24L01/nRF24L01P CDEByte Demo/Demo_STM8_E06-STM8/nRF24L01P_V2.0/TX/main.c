...
/*===========================================================================
* 函数 : LCD_Initial() => 初始化LCD                                         *
* 输入 ：mode, 当前测试模式, =0接收模式; else,发送模式                      *
* 说明 ：LCD的操作，已经被建成C库，见Oled.c文件， 提供相关显示操作， 		*
         即可调用其内部所有函数用户无需再关心LCD的寄存器操作问题。			*
============================================================================*/
void LCD_Initial(INT8U mode)
{
    LCD_Init();
	LCD_Dis_Logo();
	LCD_Dis_Str(2, 24, "STM8 board");
    LCD_Dis_Str(4, 0,  "yhmcu.taobao.com");
    if (RX == mode) { LCD_Dis_Str(6,0,"24L01+:RX 00000"); } // 接收模式
    else            { LCD_Dis_Str(6,0,"24L01+:TX 00000"); } // 发送模式        
}
...
void System_Initial(void)
{
    ...
    LCD_Initial(TX);    // 初始化OLED
    RF_Initial(TX);     // 初始化无线芯片,发送模式           
}

/*===========================================================================
* 函数 : LCD_LenDisplay() => LCD 长度显示                                   *
* 输入 ：DisplayLen, 将数据拆开显示再OLED频幕上                             *
============================================================================*/
void LCD_LenDisplay(INT16U DisplayLen)
{
    INT8U DisplayBuffer[10] = { "0000" };           // 数据分解，供显示使用

    DisplayBuffer[0] = (DisplayLen/10000) + '0';    // 取出万位
    DisplayLen %= 10000;            
    DisplayBuffer[1] = (DisplayLen/1000) + '0';     // 取出千位            
    DisplayLen %= 1000;
    DisplayBuffer[2] = (DisplayLen/100) + '0';      // 取出百位            
    DisplayLen %= 100;
    DisplayBuffer[3] = (DisplayLen/10) + '0';       // 取出十位            
    DisplayLen %= 10;
    DisplayBuffer[4] = DisplayLen + '0';            // 取出个位            
    DisplayBuffer[5] = 0;
    
    LCD_Dis_Str(6, 80, (char*)DisplayBuffer);       // 数据显示       
}
