...
namespace SerialPortTerminal
{
    ...
    {
        public Mutex gpdMutex = new Mutex();
        public double nLat = 0, nLon = 0;
        ...
        public ServiceWnd srvWnd;
        public double mLat = 0, mLon = 0;
        ...
        private void RefreshComPortList()
        {
            ...
            if (srvWnd != null && srvWnd.m_bShow)
            {
                gpdMutex.WaitOne();
                srvWnd.DrawTarget(nLat, nLon);
                gpdMutex.ReleaseMutex();
            }
        }
        ...
        {
            if (srvWnd == null)
                srvWnd = new ServiceWnd();
            srvWnd.Show();
            srvWnd.m_bShow = true;
        }

        public void ParseGGA(string data)
        {
            ...
            for (int i = 0; i < strArr.Length; i++)
            {
                ...
                if (lineArr[0] == "GPGGA")
                {
                    ...
                    {
                        //Latitude
                        Double coord = Convert.ToDouble(lineArr[2]);
                        int seconds = (int)Math.Round(coord * 3600);
                        ...
                        gpdMutex.WaitOne();
                        nLat = degrees + (minutes / 60) + (seconds / 3600);
                        gpdMutex.ReleaseMutex();

                        //Longitude
                        Double dLon = Convert.ToDouble(lineArr[4]);

                        seconds = (int)Math.Round(dLon * 3600);
                        ...
                        gpdMutex.WaitOne();
                        nLon = degrees + (minutes / 60) + (seconds / 3600);
                        gpdMutex.ReleaseMutex();
